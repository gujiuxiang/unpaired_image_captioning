        ...
        def beam_step(logprobsf, unaug_logprobsf, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state):
            ...
            for c in range(cols):  # for each column (word, essentially)
                for q in range(rows):  # for each beam expansion
                    ...
                    local_logprob = ys[q, c] if utils.under_0_4() else ys[q, c].item()
        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    for vix in range(bdash):
                        if beam_seq_table[divm][t - divm, vix] == 0 or t == self.seq_length + divm - 1:
                            final_beam = {
                                ...
                                'unaug_p': beam_seq_logprobs_table[divm][:, vix].sum() if utils.under_0_4() else beam_seq_logprobs_table[divm][:, vix].sum().item(),
                                'p': beam_logprobs_sum_table[divm][vix] if utils.under_0_4() else beam_logprobs_sum_table[divm][vix].item()
                            }
                            if max_ppl:
                    ...
                    logprobs_table[divm], state_table[divm] = self.get_logprobs_state(Variable(it.cuda()) if utils.under_0_4() else it.cuda(), *(args[divm] + [state_table[divm]]))
