...
class AttModel(CaptionModel):
    ...
    def init_hidden(self, bsz):
        weight = next(self.parameters()).data if utils.under_0_4() else next(self.parameters())
        return (Variable(weight.new(self.num_layers, bsz, self.rnn_size).zero_()) if utils.under_0_4() else weight.new_zeros(self.num_layers, bsz, self.rnn_size),
                Variable(weight.new(self.num_layers, bsz, self.rnn_size).zero_()) if utils.under_0_4() else weight.new_zeros(self.num_layers, bsz, self.rnn_size))
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        return (Variable(seq.transpose(0, 1)), Variable(seqLogprobs.transpose(0, 1))) if utils.under_0_4() else (seq.transpose(0, 1), seqLogprobs.transpose(0, 1))
    ...
    def _sample(self, fc_feats, attri_feats, att_feats, att_masks=None, opt={}):
        ...
        seq = Variable(fc_feats.data.new(batch_size, self.seq_length).long().zero_()) if utils.under_0_4() else fc_feats.new_zeros((batch_size, self.seq_length), dtype=torch.long)
        seqLogprobs = Variable(fc_feats.data.new(batch_size, self.seq_length).zero_()) if utils.under_0_4() else fc_feats.new_zeros(batch_size, self.seq_length)
        for t in range(self.seq_length + 1):
            ...
            if decoding_constraint and t > 0:
                tmp = logprobs.data.new(logprobs.size()).zero_() if utils.under_0_4() else logprobs.new_zeros(logprobs.size())
                tmp.scatter_(1, seq[:, t - 1].data.unsqueeze(1), float('-inf'))
                logprobs = (logprobs + Variable(tmp)) if utils.under_0_4() else (logprobs + tmp)
