...
class AttEnsemble(AttModel):
    ...
    def get_logprobs_state(self, it, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, state):
        ...
        logprobs = torch.stack([F.softmax(m.logit(output[i])) if utils.under_0_4() else F.softmax(m.logit(output[i]), dim=1) for i, m in enumerate(self.models)], 2).mean(2).log()
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        if utils.under_0_4():
            Variable(seq.transpose(0, 1)), Variable(seqLogprobs.transpose(0, 1))
        else:
