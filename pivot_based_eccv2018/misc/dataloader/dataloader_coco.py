...
class DataLoader_COCO(data.Dataset):
    ...
    def __init__(self, opt, train=True):
        ...
        print('DataLoader loading json file: ', opt.input_coco_json)
        self.info = json.load(open(opt.input_coco_json))
        self.ix_to_word = self.info['ix_to_word']
        self.vocab_size = len(self.ix_to_word)
        print('vocab size is ', self.vocab_size)

        # open the hdf5 file
        self.h5_label_file = h5py.File(opt.input_label_coco_h5, 'r', driver='core')

        if self.use_blob_fetcher == 0:
            self.h5_fc_file = h5py.File(opt.input_fc_coco_h5)
            #self.h5_att_file = h5py.File(opt.input_att_coco_h5)
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        fc_batch, label_batch, gts, infos = \
            zip(*sorted(zip(fc_batch, np.vsplit(label_batch, batch_size), gts, infos), key=lambda x: len(x[1]), reverse=True))
        data = {}
