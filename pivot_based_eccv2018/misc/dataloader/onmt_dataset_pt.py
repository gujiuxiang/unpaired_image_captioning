...
class onmt_dataset_pt(object):
    ...
    def _batchify(self, data, align_right=False, include_lengths=False, features=None):
        lengths = [x.size(0) for x in data]
        max_length = max(lengths)
        if features:
            num_features = len(features)
            out = data[0].new(len(data), max_length, num_features + 1).fill_(constants.PAD)
    ...
    def __getitem__(self, index):
        ...
        srcBatch, lengths = self._batchify(self.src[s:e], align_right=False, include_lengths=True,features=[f[s:e] for f in self.srcFeatures] if self.srcFeatures else None)
